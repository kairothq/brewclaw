...
export function SplitFlapAudioProvider({ children }: { children: React.ReactNode }) {
  const [isMuted, setIsMuted] = useState(false) // Sound ON by default per user request
...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  // Use amber/coffee accent for unsettled, white for settled
  const bgColor = isSettled ? "hsl(0, 0%, 0%)" : "rgba(217, 119, 6, 0.2)" // amber-600 with opacity
  const textColor = isSettled ? "#ffffff" : "#d97706" // amber-600
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden flex items-center justify-center font-heading font-bold"
      style={{
        fontSize: "clamp(4rem, 15vw, 14rem)",
        width: "0.65em",
        height: "1.05em",
